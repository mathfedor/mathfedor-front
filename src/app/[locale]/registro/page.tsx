...
export default function RegisterPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = tVal('name_required');
    }

    // Validar email
    if (!formData.email) {
      newErrors.email = tVal('email_required');
    } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
      newErrors.email = tVal('email_invalid');
    }

    // Validar contraseña
    if (!formData.password) {
      newErrors.password = tVal('password_required');
    } else if (formData.password.length < 6) {
      newErrors.password = tVal('password_min');
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!termsAccepted || !ageVerified) {
      setSubmitError(tVal('terms_required'));
      return;
    }
