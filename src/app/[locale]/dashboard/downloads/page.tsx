...
export default function DownloadsPage() {
    ...
    const loadModules = async () => {
        try {
            ...
            setAlertMessage({
                title: 'Error',
                message: error instanceof Error ? error.message : t('error_load_modules')
            });
            setIsAlertOpen(true);
        } finally {
            setIsLoading(false);
    ...
    const handleDownload = async (moduleId: string, fileIndex: number, fileName?: string) => {
        try {
            ...
            setAlertMessage({
                title: 'OK',
                message: t('success_download')
            });
            setIsAlertOpen(true);

        } catch (error) {
            console.error('Error al descargar:', error);
            const errorMessage = error instanceof Error ? error.message : t('error_download');
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-[#1E1F25] rounded-lg p-8 max-w-md mx-4 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-black dark:text-white mb-2">
                            {t('modal_customizing_title')}
                        </h3>
                        <p className="text-gray-600 dark:text-gray-400 mb-4">
                            {t('modal_customizing_desc')}
                        </p>
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="ml-3">
                                    <h3 className="text-sm font-medium text-yellow-800 dark:text-yellow-200 mb-2">
                                        {t('modal_warning_title')}
                                    </h3>
                                    <div className="text-sm text-yellow-700 dark:text-yellow-300">
                                        <p className="mb-2">
                                            {t('modal_warning_sent')}
                                        </p>
                                        <p className="font-semibold">
                                            {t('modal_warning_keep_safe')}
                                        </p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-gray-100 dark:bg-[#232323] text-left">
                                            <th className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{t('th_module')}</th>
                                            <th className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{t('th_purchase_date')}</th>
                                            <th className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{t('th_validity')}</th>
                                            <th className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{t('th_actions')}</th>
                                        </tr>
                                    </thead>
                                                ...
                                                <td className="px-6 py-4">
                                                    {expired ? (
                                                        <div className="text-sm text-gray-400 dark:text-gray-500">
                                                            <p>{t('access_expired_msg')}</p>
                                                            <p className="text-xs">{t('renew_purchase_msg')}</p>
                                                        </div>
                                                    ) : (
