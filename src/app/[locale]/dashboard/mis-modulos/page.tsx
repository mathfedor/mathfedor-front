...
export default function MisModulosPage() {
  ...
  const t = useTranslations('dashboard.mis_modulos');
      ...
      <div className="flex-1 p-8 overflow-auto">
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap gap-4 mt-4">
                <div className="flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-xl px-4 py-2 border border-white/20">
                  ...
                  <span className="text-white text-sm font-medium">{b2cCount} {t('stat_purchased')}</span>
                </div>
                <div className="flex items-center gap-2 bg-white/15 backdrop-blur-sm rounded-xl px-4 py-2 border border-white/20">
                  <FiBook className="w-4 h-4 text-purple-200" />
                  <span className="text-white text-sm font-medium">{instCount} {t('stat_institution')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex items-center bg-white/5 border border-white/10 rounded-xl p-1 gap-1">
            {[
              { key: 'all', label: t('tab_all'), count: modules.length },
              { key: 'purchase', label: t('tab_purchased'), count: b2cCount },
              { key: 'institution', label: t('tab_institution'), count: instCount },
            ].map(tab => (
              <button
                key={tab.key}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-slate-300 font-semibold text-lg mb-2">{t('empty_title')}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filtered.map((mod) => (
              <ModuleCard key={mod.module_id} module={mod} t={t} />
            ))}
          </div>
        )}
      </div>
...
function ModuleCard({ module, t }: { module: StudentModuleAccess; t: ReturnType<typeof useTranslations> }) {
      ...
      <div className="absolute top-3 right-3 z-10 flex flex-col gap-1 items-end">
        {isFreeTrialModule && module.source !== 'purchase' && (
          <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/20 text-amber-300 border border-amber-500/30">
            ...
            {t('badge_free_trial')} {FREE_TRIAL_END_DATE.toLocaleDateString()}
          </span>
        )}
      ...
      <div className="p-4">
        ...
        {module.purchaseDate && (
          <p className="text-slate-500 text-xs mb-1">
            {t('purchased_date')} {new Date(module.purchaseDate).toLocaleDateString()}
          </p>
        )}
        ...
        {expirationDate && !expired && (
          <p className="text-emerald-400 text-xs mb-3">
            {t('valid_until')} {expirationDate.toLocaleDateString()}
          </p>
        )}
        {expirationDate && expired && !isFreeTrialModule && (
          <p className="text-red-400 text-xs mb-3">
            {t('expired_on')} {expirationDate.toLocaleDateString()}
          </p>
        )}
        ...
        {module.gradeConfig?.downloadFiles?.length && !expired ? (
          <div className="flex items-center gap-1 text-xs text-slate-400 mb-3">
            ...
            <span>{module.gradeConfig.downloadFiles.length} {t('files_available')}</span>
          </div>
        ) : null}
