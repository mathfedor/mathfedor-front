...
const DOCUMENT_OPTIONS = [
  { slug: 'terminos-y-condiciones', titleKey: 'doc_terms' as const },
  { slug: 'politica-privacidad', titleKey: 'doc_privacy' as const },
  { slug: 'autorizacion-datos-menor', titleKey: 'doc_minor_data' as const },
  { slug: 'politica-cookies', titleKey: 'doc_cookies' as const },
];
...
export default function AdminLegalPage() {
  ...
  const t = useTranslations('dashboard.legal');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!slug) {
      setFeedback({ type: 'error', message: t('error_select_type') });
      return;
    }
    if (!file) {
      setFeedback({ type: 'error', message: t('error_select_file') });
      return;
    }
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    <select
                      ...
                      required
                    >
                      <option value="">{t('select_doc_type')}</option>
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        {file ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-gray-500 mt-1">{t('pdf_hint')}</p>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  >
                    {submitting ? t('btn_uploading') : t('btn_upload')}
                  </button>
                </form>
              </div>
                              ...
                              {currentDoc ? (
                                <div className="mt-1 space-y-1">
                                  <p className="text-xs text-gray-500 dark:text-gray-400">
                                    {t('current_version')} <span className="font-bold text-orange-600 dark:text-orange-400">{currentDoc.version}</span>
                                  </p>
                                  <p className="text-xs text-gray-400 dark:text-gray-500">
                                    {t('updated_at')} {formatDate(currentDoc.updatedAt)}
                                  </p>
                                  <div className="flex items-center gap-3 mt-2">
                                    ...
                                    {hasHistory && (
                                      ...
                                      >
                                        ...
                                        {t('history_btn')} ({currentDoc.versions!.length})
