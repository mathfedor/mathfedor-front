...
export default function LoginForm() {
  ...
  const handleGoogleSignIn = useCallback(async (response: GoogleCredentialResponse) => {
    ...
    try {
      ...
      if (!result.ok) {
        setError(result.message || 'Error en la autenticación con Google');
        return;
      }
  ...
  const initializeGoogleSignIn = useCallback(() => {
    if (window.google) {
      window.google.accounts.id.initialize({
        client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || 'TU_GOOGLE_CLIENT_ID',
  ...
  useEffect(() => {
    // Cargar la API de Google Identity
    const loadGoogleAPI = () => {
  ...
  const handleGoogleClick = () => {
    if (window.google && window.google.accounts) {
      // Abrir el popup de Google Identity
      window.google.accounts.id.prompt();
    } else {
      setError('Error al cargar la autenticación de Google. Asegúrate de tener configurado el Google Client ID.');
    }
  };
    ...
    <div className="min-h-screen flex flex-col">
      <div className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
        <div className="max-w-md w-full space-y-8">
          <div>
            <h2 className="mt-6 text-center text-3xl font-bold text-gray-900">Inicia sesión en tu cuenta</h2>
            <p className="mt-2 text-center text-sm text-gray-600">
              O{' '}
              <Link href="/registro" className="font-medium text-orange-600 hover:text-orange-500">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="rounded-md shadow-sm -space-y-px">
              <div>
                ...
                <input 
                  ...
                  autoComplete="email" 
                  required 
                  className="appearance-none rounded-none relative block w-full px-3 py-4 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-orange-500 focus:border-orange-500 focus:z-10 sm:text-sm" 
              ...
              <div>
                ...
                <input 
                  ...
                  required 
                  className="appearance-none rounded-none relative block w-full px-3 py-4 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-orange-500 focus:border-orange-500 focus:z-10 sm:text-sm" 
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <Link href="/recuperar-password" className="font-medium text-orange-600 hover:text-orange-500">
