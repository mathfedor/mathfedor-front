  ...
  return (
    <div className="screen active" id="screen-unit">
      ...
      <div className={`unit-hero-band ${unit.heroCls}`}>
        ...
        <div className="uhb-title" style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <span>{unit.short} — {unit.name}</span>
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
              fedorTTS.speak(`${unit.name}. ${unit.std}`);
            }}
            className="bg-white/20 hover:bg-white/30 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs shadow border border-white/40 transition-transform active:scale-90"
      ...
      <div className="topics-list">
        {unit.topics.map((topic, ti) => (
          <div key={topic.id} style={{ display: 'contents' }}>
            <div style={{ fontSize: '12px', fontWeight: 900, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.08em', margin: '1.25rem 0 .4rem', paddingLeft: '4px', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>{topic.icon} {topic.title}</span>
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  fedorTTS.speak(`${topic.title}. ${topic.desc || ''}`);
                }}
                className="bg-purple-100 hover:bg-purple-200 text-purple-800 rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-xs border border-purple-200 transition-transform active:scale-90"
                title="Escuchar tema"
              >
                🔊
              </button>
